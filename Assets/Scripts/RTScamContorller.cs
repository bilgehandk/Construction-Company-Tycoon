...
public class RTScamContorller : MonoBehaviour
{
#if UNITY_EDITOR
    ...
    private void Update()
    {
        ...
        if (Input.touchCount >= 1)
        {
            ...
            if (Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                ...
                Camera.transform.position = new Vector3(
                    Mathf.Clamp(Camera.transform.position.x, MIN_X, MAX_X),
                    Mathf.Clamp(Camera.transform.position.y, MIN_Y, MAX_Y),
                    Mathf.Clamp(Camera.transform.position.z, MIN_Z, MAX_Z));
        ...
        if (Input.touchCount >= 2)
        {
            ...
            //Lower (Zoom in)
            if (Camera.transform.position.y < (cameraStartPosition.y - CameraLowerHeightBound) || Camera.transform.position.y <= 10)
            {
                Camera.transform.position = camPositionBeforeAdjustment;
            }
